...
;~ showDialog(1, "{00FF00}[ADJUTANT]{FFFFFF} - ЗАГРУЗКА МЕНЮ", "{FFFFFF}Пожалуйста подождите того момента как загрузка закончиться. Просто ждите =)", "X")
;~ SendInput, ===========25`%
;~ sleep, 50
;~ SendInput, {ENTER}
;~ showDialog(1, "{00FF00}[ADJUTANT]{FFFFFF} - ЗАГРУЗКА МЕНЮ", "{FFFFFF}Пожалуйста подождите того момента как загрузка закончиться. Просто ждите =)", "X")
...
sleep, 50
;~ SendInput, {Enter}
;~ showDialog(0, "{00FF00}[ADJUTANT]{FFFFFF} - АВТОРИЗАЦИЯ", "{FFFFFF}Нажмите {00FF00}<ENTER>{FFFFFF} для входа в менюю. Для выхода нажмите {FF0000}<ESC> {FFFFFF}(ESCAPE).", "НЕ ТРОЖЬ")
;~ input, autorization, V, {Enter}.{Escape}
...
if (menu == 1)
{
    ...
    if Organ = SAPD
    {
        ...
        {
        showDialog(1, "{00FF00}[ADJUTANT]{FFFFFF} - СМЕНА МАРКИРОВКИ", "{FFFFFF}Введите новую маркировку, по формату: {00FFFF}цифра-БУКВА-цифра`n{FFFFFF}Для того что-бы принять изменения введите маркировку, после чего нажмите{00FFFF} <Enter>", "X")
        SendMessage, 0x50,, 0x4090409,, A
        input, newmarking, V, {Enter}.{Escape}
            if ErrorLevel = EndKey:Enter
            {
            addChatMessage("{00FF00}[ADJUTANT]{FFFFFF} - Вы удачно сменили свою маркировку на - " newmarking ".")
                Sleep, 1500
                SendChat("/r " marking " to DISP: 10-27, on " newmarking ", RFR, OVER SAY **")
                marking:=newmarking
                menu := 0
                return
            }
            ...
            {
            addChatMessage("{00FF00}[ADJUTANT]{FFFFFF} - Вы выйшли из меню смены маркировки, Ваша маркировка - " marking ".")
                newmarking:=-1
                menu := 0
                return
            }
        ...
        if ErrorLevel = EndKey:Escape
        {
            addChatMessage("{00FF00}[ADJUTANT]{FFFFFF} - Вы выйшли из меню 'смена значения для команды /limit'. Новое значение не указано!")
        }
        else
        if ErrorLevel = EndKey:Enter
        {
            addChatMessage("{00FF00}[ADJUTANT]{FFFFFF} - Нажмите пожалуйста{32CD32} <Insert>{FFFFFF} что-бы применить настройки.")
        }
        menu:=0
        return
    }
    ...
    if Organ = FBI
    {
        ...
        if ErrorLevel = EndKey:Insert
        {
            ...
            limit:=newlimit
            addChatMessage("{00FF00}[ADJUTANT]{FFFFFF} - Вы установили новое значение для команды /limit - {00FFFF}" limit "{FFFFFF} км\час.")
            IniWrite, %limit%, %A_MyDocuments%\GTA San Andreas User Files\SAMP\hotkeys.ini, List, limit
            return
        }
        else
        if ErrorLevel = EndKey:Escape
    ...
    if Organ = CIA
    {
        ...
        input, newmarking, V, {Enter}.{Escape}
            ...
            addChatMessage("{00FF00}[ADJUTANT]{FFFFFF} - Вы удачно сменили свою маркировку на - " newmarking ".")
                ...
                SendChat("/r " marking " to DISP: 10-27, на " newmarking ", доступен, конец связи **")
                }
                marking:=newmarking
                menu := 0
                return
...
else if (menu == 6)
{
    ...
    {
        IniWrite, %partnerSurname%, %A_ScriptDir%\partner.ini, Partner, Surname
        IniWrite, %partnerId%, %A_ScriptDir%\partner.ini, Partner, Id
        IniWrite, %partnerPost%, %A_ScriptDir%\partner.ini, Partner, Post
    ...
    {
        IniWrite, None, %A_ScriptDir%\partner.ini, Partner, Surname
        IniWrite, None, %A_ScriptDir%\partner.ini, Partner, Id
        IniWrite, None, %A_ScriptDir%\partner.ini, Partner, Post
        partner:=0
        partnersurname:="None"
        partnerid:="None"
        partnerpost:="None"
